...
class HomeController extends Controller
{
    ...
    //       return redirect(route('home'))->with('warning', 'Format berkas file tidak benar, file harus berformat .xlsx!');
    //   }
    //   Excel::load($request->file('excel'), function ($reader) {
    //       $reader->each(function ($sheet) {
    //         if (empty($sheet->tanggal) || empty($sheet->sembuh)) {
    //         }else {
    //           $data = Dataset::updateOrCreate(['tanggal' => $sheet->tanggal], ['sembuh' => $sheet->sembuh]);
    //         }
    //       });
    //   });
    //   return redirect(route('home'))->with('success', 'Import data berhasil!');
    // }
    ...
    //           if ( $key->sembuh >= $key2['bawah'] && $key->sembuh < $key2['atas']) {
    //             $fuzzyfikasi = $y;
    //           }
    //         $y++;
    //         }
    ...
    //       $data[$i]=['tanggal'=>$key->tanggal, 'sembuh'=>$key->sembuh, 'fuzzifikasi'=>$fuzzyfikasi,'orde'=>$object];
    //
    //     }else{
    //       foreach ($interval as $key2) {
    //         if ( $key->sembuh >= $key2['bawah'] && $key->sembuh < $key2['atas']) {
    ...
    //         $y++;
    //       }
    //     $data[$i]=['tanggal'=>$key->tanggal, 'sembuh'=>$key->sembuh, 'fuzzifikasi'=>$fuzzyfikasi,'orde'=>null];
    //     }
    //     $i++;
    //   }
    //   return $data;
    ...
    //     }
    //
    //     $relasi[$i]=['tanggal'=>$key['tanggal'], 'sembuh'=>$key['sembuh'], 'fuzzifikasi'=>$key['fuzzifikasi'],'orde'=>$key['orde'], 'relasi'=>$temp];
    //     $i++;
    //   }
    //   return $relasi;
    // }
    ...
    //           $prediksi[$i]=['tanggal'=>$key['tanggal'], 'sembuh'=>$key['sembuh'], 'fuzzifikasi'=>$key['fuzzifikasi'],'orde'=>$key['orde'], 'relasi'=>$key['relasi'],'ramalan'=>$key2['ramalan']];
    //         }
    //       }
    //     }else {
    //       $prediksi[$i]=['tanggal'=>$key['tanggal'], 'sembuh'=>$key['sembuh'], 'fuzzifikasi'=>$key['fuzzifikasi'],'orde'=>$key['orde'], 'relasi'=>$key['relasi'],'ramalan'=>null];
    //     }
    //     $i++;
    //   }
    //   return $prediksi;
    ...
    //       $absolute = abs($key['sembuh']-$key['ramalan']);
    //       $afer = $absolute/$key['sembuh'];
    //       $mse = pow($absolute,2);
    //     }else{
    //       $absolute = null;
    //       $afer = null;
    //       $mse =null;
    //     }
    //     $error[$i]=['absolute'=>$absolute,'afer'=>$afer,'mse'=>$mse,'tanggal'=>$key['tanggal'], 'sembuh'=>$key['sembuh'], 'fuzzifikasi'=>$key['fuzzifikasi'],'orde'=>$key['orde'], 'relasi'=>$key['relasi'],'ramalan'=>$key['ramalan']];
    //     $i++;
    //   }
    //   return $error;
    // }
    ...
    //   $dataset=Dataset::all();
    //   if ($jumlah_data==0) {
    //     return redirect(route('home'))->with('warning', 'Dataset belum ada!');
    //   }
    //   $data['dataset'] = $dataset;
    //   $data['user'] = User::find(Auth::user()->id);
    //   $k=(int)round(1+3.3*log10($jumlah_data));
    //   $data['k']=$k;
    //   $max=Dataset::max('sembuh');
    //   $min=Dataset::min('sembuh');
